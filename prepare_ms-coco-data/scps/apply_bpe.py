...
class BPE(object):
    def __init__(self, codes, merges=-1, separator="@@", vocab=None, glossaries=None):
        ...
        self.bpe_codes = [
            tuple(item.strip("\r\n ").split(" "))
            for (n, item) in enumerate(codes.read().rstrip("\n").split("\n"))
    ...
    def process_lines(self, fobj, dropout=0, num_workers=1):
        ...
        if num_workers >= 1:
            ...
            for i in range(num_workers):
                ...
                pool.apply_async(
                    _process_lines, (self, list(chunks[i]), tmp.name, dropout)
                )
            pool.close()
            pool.join()
            for i in range(num_workers):
...
def _process_lines(bpe, infile, outfile, dropout):
...
def apply_bpe(codes, infile, num_workers=1):

    if num_workers <= 0:
        num_workers = cpu_count()

    bpe = BPE(codes)
    return bpe.process_lines(infile, 0, num_workers)
