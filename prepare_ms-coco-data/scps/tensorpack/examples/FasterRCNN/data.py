...
def print_class_histogram(roidbs):
    ...
    data = list(
        itertools.chain(*[[class_names[i + 1], v] for i, v in enumerate(gt_hist[1:])])
    )
    COL = min(6, len(data))
...
class TrainingDataPreprocessor:
    ...
    def __call__(self, roidb):
        ...
        if len(boxes):
            assert (
                klass.max() <= self.cfg.DATA.NUM_CATEGORY
        ...
        try:
            if self.cfg.MODE_FPN:
                ...
                for i, (anchor_labels, anchor_boxes) in enumerate(
            ...
            log_once(
                "Input {} is filtered for training: {}".format(fname, str(e)), "warn"
            )
            return None
        ...
        if self.cfg.MODE_MASK:
            ...
            segmentation = [
                segmentation[k] for k in range(len(segmentation)) if not is_crowd[k]
            ]
    ...
    def get_anchor_labels(self, anchors, gt_boxes, crowd_boxes):
        ...
        if crowd_boxes.size > 0:
            ...
            overlap_with_crowd = cand_inds[
                ioas.max(axis=0) > self.cfg.RPN.CROWD_OVERLAP_THRESH
            ]
...
def get_eval_dataflow(name, shard=0, num_shards=1):
    ...
    ds = DataFromListOfDict(
        roidbs[img_range[0] : img_range[1]], ["file_name", "image_id"]
    )
