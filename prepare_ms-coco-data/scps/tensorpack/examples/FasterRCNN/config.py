...
_C.BACKBONE.FREEZE_AT = (
    2  # options: 0, 1, 2. How many stages in backbone to freeze (not training)
)
...
_C.TRAIN.BASE_LR = (
    1e-2  # defined for total batch size=8. Otherwise it will be adjusted automatically
)
_C.TRAIN.WARMUP = 1000  # in terms of iterations. This is not affected by #GPUs
_C.TRAIN.WARMUP_INIT_LR = (
    0.0  # defined for total batch size=8. Otherwise it will be adjusted automatically
)
_C.TRAIN.STEPS_PER_EPOCH = 500
_C.TRAIN.STARTING_EPOCH = (
    1  # the first epoch to start with, useful to continue a training
)
...
_C.RPN.BATCH_PER_IM = (
    256  # total (across FPN levels) number of anchors that are marked valid
)
_C.RPN.MIN_SIZE = 0
...
_C.RPN.TEST_POST_NMS_TOPK = (
    1000  # if you encounter OOM in inference, set this to a smaller number
)
...
def finalize_configs(is_training):
    ...
    if _C.MODE_FPN:
        size_mult = _C.FPN.RESOLUTION_REQUIREMENT * 1.0
    ...
    if is_training:
        ...
        if (
            isinstance(train_scales, (list, tuple))
            and train_scales[1] - train_scales[0] > 100
        ):
        ...
        if _C.TRAINER == "horovod":
            ...
            ngpu = hvd.size()
            logger.info(
                "Horovod Rank={}, Size={}, LocalRank={}".format(
        ...
        assert (
            ngpu % 8 == 0 or 8 % ngpu == 0
        ), "Can only train with 1,2,4 or >=8 GPUs, but found {} GPUs".format(ngpu)
    else:
