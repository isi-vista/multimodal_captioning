...
FeatexResult = namedtuple("FeatexResult", ["box", "score", "class_id", "feature"])
...
def extract_features(img, model_func):
    """
    ...
    boxes, probs, labels, features = model_func(resized_img)
    ...
    results = [
        dict(FeatexResult(*args)._asdict()) for args in zip(boxes, probs, labels.tolist(), features)
    ]
    return results
...
def predict_image(img, model_func):
    ...
    if masks:
        full_masks = [
            _paste_mask(box, mask, orig_shape) for box, mask in zip(boxes, masks[0])
        ]
        masks = full_masks
    else:
        # fill with none
    ...
    results = [
        DetectionResult(*args) for args in zip(boxes, probs, labels.tolist(), masks)
    ]
    return results
...
def multithread_predict_dataflow(dataflows, model_funcs):
    ...
    with ThreadPoolExecutor(max_workers=num_worker, **kwargs) as executor, tqdm.tqdm(
        total=sum([df.size() for df in dataflows])
    ) as pbar:
        futures = []
