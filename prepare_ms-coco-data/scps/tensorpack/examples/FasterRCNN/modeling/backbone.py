...
def backbone_scope(freeze):
    ...
    with argscope(
        [Conv2D, MaxPooling, BatchNorm], data_format="channels_first"
    ), argscope(
        Conv2D,
        use_bias=False,
        activation=nonlin,
        kernel_initializer=tf.keras.initializers.VarianceScaling(  # kernel_initializer=tf.variance_scaling_initializer(
...
def get_norm(zero_init=False):
    ...
    if cfg.BACKBONE.NORM == "GN":
        ...
        Norm = BatchNorm
        layer_name = "bn"
    return lambda x: Norm(
        layer_name, x, gamma_initializer=tf.zeros_initializer() if zero_init else None
    )
...
def resnet_bottleneck(l, ch_out, stride):
    ...
    if cfg.BACKBONE.STRIDE_1X1:
        ...
        if stride == 2:
            l = tf.pad(
                l, [[0, 0], [0, 0], maybe_reverse_pad(0, 1), maybe_reverse_pad(0, 1)]
            )
...
def resnet_c4_backbone(image, num_blocks):
    ...
    with backbone_scope(freeze=freeze_at > 0):
        l = tf.pad(
            image, [[0, 0], [0, 0], maybe_reverse_pad(2, 3), maybe_reverse_pad(2, 3)]
        )
...
def resnet_fpn_backbone(image, num_blocks):
    ...
    new_shape2d = tf.cast(
        tf.math.ceil(tf.cast(shape2d, tf.float32) / mult) * mult, tf.int32
    )
