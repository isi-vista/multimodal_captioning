...
class CascadeRCNNHead(object):
    def __init__(
        ...
        with tf.compat.v1.variable_scope("cascade_rcnn_stage1"):
            H1, B1, F1 = self.run_head(self.proposals, 0)

        with tf.compat.v1.variable_scope("cascade_rcnn_stage2"):
            B1_proposal = self.match_box_with_gt(B1, ious[1])
            H2, B2, F2 = self.run_head(B1_proposal, 1)

        with tf.compat.v1.variable_scope("cascade_rcnn_stage3"):
            B2_proposal = self.match_box_with_gt(B2, ious[2])
            H3, B3, F3 = self.run_head(B2_proposal, 2)
        self._cascade_boxes = [B1, B2, B3]
        self._cascade_features = [F1, F2, F3]
    ...
    def run_head(self, proposals, stage):
        ...
        label_logits, box_logits, features = fastrcnn_outputs(
            "outputs", head_feature, self.num_categories, class_agnostic_regression=True
        )
        head = FastRCNNHead(
            proposals, box_logits, label_logits, features, self.gt_boxes, reg_weights
        )
        ...
        return head, tf.stop_gradient(refined_boxes, name="output_boxes"), features
        ...
        if self.training:
            with tf.name_scope("match_box_with_gt_{}".format(iou_threshold)):
                ...
                labels_per_box = tf.stop_gradient(
                    labels_per_box * tf.cast(fg_mask, tf.int64)
                )
    ...
    def output_features(self, name=None):
        """
        Returns:
            Nx1024
        """
        ret = tf.concat(self._cascade_features, axis=-1)
        return ret
