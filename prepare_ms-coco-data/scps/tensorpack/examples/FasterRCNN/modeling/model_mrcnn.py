...
def maskrcnn_loss(mask_logits, fg_labels, fg_target_masks):
    ...
    if get_tf_version_tuple() >= (1, 14):
        ...
        indices = tf.stack(
            [tf.range(tf.size(fg_labels, out_type=tf.int64)), fg_labels - 1], axis=1
        )  # #fgx2
    ...
    pos_accuracy = tf.reduce_mean(
        tf.cast(pos_accuracy, tf.float32), name="pos_accuracy"
    )
    fg_pixel_ratio = tf.reduce_mean(
        tf.cast(truth_label, tf.float32), name="fg_pixel_ratio"
    )
...
def maskrcnn_upXconv_head(feature, num_category, num_convs, norm=None):
    ...
    with argscope(
        ...
        for k in range(num_convs):
            l = Conv2D(
                "fcn{}".format(k), l, cfg.MRCNN.HEAD_DIM, 3, activation=tf.nn.relu
            )
            if norm is not None:
