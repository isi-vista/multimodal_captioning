...
def fastrcnn_outputs(feature, num_categories, class_agnostic_regression=False):
    ...
    return classification, box_regression, feature
...
def fastrcnn_losses(labels, label_logits, fg_boxes, fg_box_logits):
    ...
    with tf.name_scope("label_metrics"), tf.device("/cpu:0"):
        ...
        false_negative = tf.where(
            empty_fg,
            0.0,
            tf.cast(tf.truediv(num_zero, num_fg), tf.float32),
            name="false_negative",
        )
...
@under_name_scope()
def fastrcnn_predictions(boxes, scores, features):
    """
    ...
    class_num = boxes.shape[1]

    # expand features dimension to match boxes & scores dimension
    features = tf.tile(tf.expand_dims(features, axis=1), [1, class_num, 1])

    boxes = tf.transpose(boxes, [1, 0, 2])[1:, :, :]  # #catxnx4
    scores = tf.transpose(scores[:, 1:], [1, 0])  # #catxn
    features = tf.transpose(features, [1, 0, 2])[1:, :, :] # #catxnxdim
    ...
    filtered_features = tf.gather_nd(features, filtered_ids) # Fxdim
    ...
    selection = tf.image.non_max_suppression(
        nms_boxes, filtered_scores, cfg.TEST.RESULTS_PER_IM, cfg.TEST.FRCNN_NMS_THRESH
    )
    ...
    #final_features = tf.gather(filtered_features, tf.cast(selection/class_num, tf.int32), name="features")
    final_features = tf.gather(filtered_features, selection, name="features")
    return final_boxes, final_scores, final_labels, final_features
...
class FastRCNNHead(object):
    ...
    def __init__(
        self, proposals, box_logits, label_logits, features, gt_boxes, bbox_regression_weights
    ):
        """
        Args:
            ...
            features: Nxd, the output of the head
            gt_boxes: Mx4
    ...
    def output_features(self):
        """ Returns: N x 1024 feature tensor. """
