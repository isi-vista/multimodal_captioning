...
class GeneralizedRCNN(ModelDesc):
    ...
    def get_inference_tensor_names(self):
        ...
        Returns:
            ...
            [str]: output names
        """
        out = ["output/boxes", "output/scores", "output/labels", "output/features"]
        if cfg.MODE_MASK:
    ...
    def build_graph(self, *inputs):
        ...
        if "gt_masks_packed" in inputs:
            gt_masks = tf.cast(
                unpackbits_masks(inputs.pop("gt_masks_packed")),
                tf.uint8,
                name="gt_masks",
            )
        ...
        targets = [
            inputs[k] for k in ["gt_boxes", "gt_labels", "gt_masks"] if k in inputs
        ]
        ...
        if self.training:
            ...
            for name in self.get_inference_tensor_names()[1]:
                try:
                    ...
                    raise KeyError(
                        "Your model does not define the tensor '{}' in inference context.".format(
                            name
                        )
                    )
...
class ResNetC4Model(GeneralizedRCNN):
    def inputs(self):
        ret = [
            ...
            tf.TensorSpec(
                (None, None, cfg.RPN.NUM_ANCHOR, 4), tf.float32, "anchor_boxes"
            ),
    ...
    def rpn(self, image, features, inputs):
        ...
        proposal_boxes, proposal_scores = generate_rpn_proposals(
            ...
            if self.training
            else cfg.RPN.TEST_POST_NMS_TOPK,
        )
    ...
    def roi_heads(self, image, features, proposals, targets):
        ...
        (
            ...
            fastrcnn_box_logits,
            fastrcnn_features,
        ) = fastrcnn_outputs("fastrcnn", feature_gap, cfg.DATA.NUM_CATEGORY)
        ...
        if self.training:
            ...
            if cfg.MODE_MASK:
                ...
                target_masks_for_fg = tf.squeeze(
                    target_masks_for_fg, 1, "sampled_fg_mask_targets"
                )
                all_losses.append(
                    maskrcnn_loss(
                        mask_logits, proposals.fg_labels(), target_masks_for_fg
                    )
                )
            return all_losses
        else:
            ...
            decoded_features = fastrcnn_head.output_features(name="fastrcnn_all_features")
            final_boxes, final_scores, final_labels, final_features = fastrcnn_predictions(
                decoded_boxes, label_scores, decoded_features, name_scope="output"
            )

            if cfg.MODE_MASK:
                roi_resized = roi_align(
                    featuremap, final_boxes * (1.0 / cfg.RPN.ANCHOR_STRIDE), 14
                )
    ...
    def inputs(self):
        ...
        for k in range(len(cfg.FPN.ANCHOR_STRIDES)):
            ret.extend(
                [
                    tf.TensorSpec(
                        (None, None, num_anchors),
                        tf.int32,
                        "anchor_labels_lvl{}".format(k + 2),
                    ),
                    tf.TensorSpec(
                        (None, None, num_anchors, 4),
                        tf.float32,
