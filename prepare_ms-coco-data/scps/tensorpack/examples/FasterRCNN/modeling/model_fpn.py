    ...
    with argscope(
        ...
        for idx, lat in enumerate(lat_2345[::-1]):
            if idx == 0:
                ...
                lat = lat + upsample2x(
                    "upsample_lat{}".format(6 - idx), lat_sum_5432[-1]
                )
                lat_sum_5432.append(lat)
        p2345 = [
...
def fpn_map_rois_to_levels(boxes):
    ...
    level = tf.cast(
        tf.floor(4 + tf.math.log(sqrtarea * (1.0 / 224) + 1e-6) * (1.0 / np.log(2))),
        tf.int32,
    )
...
def generate_fpn_proposals(
    ...
    if cfg.FPN.PROPOSAL_MODE == "Level":
        fpn_nms_topk = (
            cfg.RPN.TRAIN_PER_LEVEL_NMS_TOPK
            if training
            else cfg.RPN.TEST_PER_LEVEL_NMS_TOPK
        )
        for lvl in range(num_lvl):
