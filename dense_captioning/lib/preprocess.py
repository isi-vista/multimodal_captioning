...
class VGDataProcessor:
    def __init__(self, split_name, image_data, regions_all=None, vocab=None,
        ...
        tic = time.time()
        for i in tqdm(range(len(image_data)), desc='%s' % split_name):
            ...
            for obj in item['regions']:
                ...
                obj['phrase_tokens'] = phrase.translate(str.maketrans('','', string.punctuation)).split()
    ...
    def init_vocabulary(self, phrases_all):
        ...
        print("Found %d unique word tokens." % len(list(word_freq.items())))
