...
class VgEvalCap:
  ...
  def evaluate(self):
    ...
    for imgId in imgIds:
      ...
      match_pairs = list(zip(match_model_ids, match_gt_ids[match_model_ids]))
    ...
    all_match_ratios = np.concatenate([v['match_ratios'] for k, v in eval_stats.items()])
    all_probs = np.concatenate([v['probs'] for k, v in eval_stats.items()])
    all_scores = np.concatenate([v['meteor_scores'] for k, v in eval_stats.items()])
