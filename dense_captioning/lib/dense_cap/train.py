...
# This file is adapted from Linjie and Xinlei's work
# --------------------------------------------------------
# Fast R-CNN
...
class SolverWrapper(object):
    ...
    def get_variables_in_checkpoint_file(self, file_name):
        try:
            # reader = tensorflow.NewCheckpointReader(file_name)
        ...
        with sess.graph.as_default():
            ...
            if cfg.TRAIN.OPTIMIZER == "sgd_m":
                self.optimizer = tf.train.MomentumOptimizer(
                    learning_rate, cfg.TRAIN.MOMENTUM
                )
            ...
            # gradient clipping
            capped_gvs = [
                (tf.clip_by_norm(grad, cfg.TRAIN.CLIP_NORM), var) for grad, var in gvs
            ]
            ...
            if cfg.TRAIN.DOUBLE_BIAS:
                ...
                train_op = self.optimizer.apply_gradients(
                    final_gvs, global_step=self.global_step
                )
            else:
                train_op = self.optimizer.apply_gradients(
    ...
    def find_previous(self):
        sfiles = os.path.join(
            self.output_dir, cfg.TRAIN.SNAPSHOT_PREFIX + "_iter_*.ckpt.meta"
        )
        ...
        nfiles = os.path.join(
            self.output_dir, cfg.TRAIN.SNAPSHOT_PREFIX + "_iter_*.pkl"
        )
    ...
    def initialize(self, sess):
        ...
        for stepsize in cfg.TRAIN.STEPSIZE:
            if last_snapshot_iter >= stepsize:
                ...
                print(
                    "Decrease learning rate by gamma {}, changed to rate: {}, stepsize:{}, iters:{}".format(
                        cfg.TRAIN.GAMMA, rate, stepsize, last_snapshot_iter
                    )
                )
            else:
        ...
        while iters < max_iters + 1:
            ...
            if cfg.TRAIN.LR_DIY_DECAY:
                ...
                if iters % cfg.TRAIN.STEPSIZE[0] == 1 and iters > 1:
                    ...
                    sess.run(tf.assign(lr, rate))
                    print(
                        "Decrease learning rate to {}, with iters: {}, stepsize: {}, \
                        gamma: {}".format(
                            rate, iters, cfg.TRAIN.STEPSIZE[0], cfg.TRAIN.GAMMA
                        )
                    )
                    next_stepsize = stepsizes.pop()
    ...
    def vis_regions(self, im, regions, iter_n, save_path="debug"):
        ...
        for i in xrange(len(regions)):
            ...
            im_new = np.copy(im)
            cv2.rectangle(
                im_new, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 0, 255), 2
            )
