...
class BeamSearch(object):
    ...
    def apply(self, visual, cls, seqs_inp, out_size=1, return_probs=False, **kwargs):
        ...
        with self.model.statefulness(self.b_s):
            for t in range(self.max_len):
                visual, outputs = self.iter(
                    t, visual, cls, seqs_inp, indices_2d, indices_3d, outputs, return_probs, **kwargs
                )
    ...
    def iter(self, t: int, visual, cls, seqs_inp, indices_2d, indices_3d, outputs, return_probs, **kwargs):
        cur_beam_size = 1 if t == 0 else self.beam_size

        word_logprob = self.model.step(
            t, self.selected_words, visual, cls, seqs_inp, None, mode="feedback", training=self.training, **kwargs
        )
