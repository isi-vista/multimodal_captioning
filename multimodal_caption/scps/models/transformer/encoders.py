...
class TextEncoder(tf.keras.layers.Layer):
    def __init__(self, target_vocab_size, max_len, padding_idx=0, num_layers=3, d_model=512, num_heads=8, dff=2048, rate=0.1):
...
class VisualEncoder(tf.keras.layers.Layer):
    def __init__(self, padding_idx=0, num_layers=3, d_model=512, num_heads=8, dff=2048, rate=0.1):
        ...
        self.enc_layers_x = [EncoderLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)]
        self.enc_layers_y = [EncoderLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)]
        self.enc_layers_xy = [EncoderLayer(d_model, num_heads, dff, rate) for _ in range(num_layers)]
