...
class COCODataLoader:
    def __init__(self, caption_file, feature_file, class_file, batch_size=10):
        self.load_dataset(caption_file, feature_file, class_file)
        self.data = self.refresh(batch_size)

    def get(self):
        return self.data

    def load_dataset(self, caption_file, feature_file, class_file):
    ...
    def dataset_map_func(self, im, en_seq, de_seq, score):
        ...
        return np.array(feat), cls, en_seq, de_seq, caps, score
    ...
    def refresh(self, batch_size=64, sample_num=None):
        ...
        data = data.map(
            lambda im, en_seq, de_seq, score: tf.numpy_function(
                ...
                [tf.float32, tf.int32, tf.int32, tf.int32, tf.string, tf.float32],
            ),
...
class Multi30kDataLoader:
    ...
    def dataset_map_func(self, image):
        ...
        return np.array(feat), cls, en_seqs[0], de_seqs[0], de_caps[0], np.float32(1.0)
