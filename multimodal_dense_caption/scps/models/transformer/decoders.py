...
class DecoderLayer(StatefulModel):
    def __init__(self, d_model=512, num_heads=8, dff=2048, rate=0.1):
        ...
        self.caps_enc_att = MultiHeadAttention(
            d_model, num_heads, can_be_stateful=False
        )
        ...
        self.caps_alpha = tf.keras.layers.Dense(d_model)
    ...
    def call(
        ...
        # attend to dense captions
        caps_enc_attn_outputs = []
        for caps_enc in caps_enc_output:
            caps_enc_attn_output, caps_enc_attn_weights = self.caps_enc_att(
                caps_enc, caps_enc, self_attn_output, None
            )
            caps_enc_attn_output = self.dropout3(
                caps_enc_attn_output, training=training
            )
            caps_enc_attn_output = self.layernorm3(
                caps_enc_attn_output + self_attn_output
            )
            caps_alpha = tf.math.sigmoid(
                self.caps_alpha(
                    tf.concat([caps_enc_attn_output, self_attn_output], axis=-1)
                )
            )
            caps_enc_attn_output = tf.multiply(caps_alpha, caps_enc_attn_output)
            caps_enc_attn_outputs.append(caps_enc_attn_output)
        ...
        text_enc_attn_output = self.dropout4(text_enc_attn_output, training=training)
        text_enc_attn_output = self.layernorm4(
        ...
        merged_enc_attn_output = tf.concat(
            [
                visual_enc_attn_output,
                text_enc_attn_output,
                tf.concat(tf.unstack(caps_enc_attn_outputs), axis=-1),
            ],
            axis=-1,
        )
        ...
        ffn_output = self.dropout5(ffn_output, training=training)
        output = self.layernorm5(
