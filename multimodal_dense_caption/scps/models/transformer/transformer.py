...
class Transformer(StatefulModel):
    def __init__(
        ...
        # self.register_state("caps_enc_output", None)
        self.register_state("text_enc_output", None)
        self.register_state("text_enc_mask", None)

        # hard-coded for dense captioning inputs (3 captions)
        self.caps_enc_outputs = [
            "caps_enc_output1",
            "caps_enc_output2",
            "caps_enc_output3",
        ]
        for item in self.caps_enc_outputs:
            self.register_state(item, None)
    ...
    def call(self, visual, cls, caps, text, seq, training=False):
        # image features, without masking
        visual_enc_output, visual_enc_mask = self.visual_encoder(
            visual, cls, training
        )  # (batch_size, inp_seq_len, d_model)

        # word embedding for dense captions
        caps_enc_output = []
        for c in caps:
            c_embedding = self.embedding(c)
            c_enc_output, _ = self.text_encoder(c_embedding, None, training)
            caps_enc_output.append(c_enc_output)

        # word embedding for (English) text encoder
        text_enc_mask, _, _ = self.create_mask(text)
        text_embedding = self.embedding(text)

        # text self-attention encoding
    ...
    def step(
        ...
        **kwargs
        ...
        if mode == "teacher_forcing":
            ...
            if t == 0:
                ...
                for i, c in enumerate(caps):
                    c_embedding = self.embedding(c)
                    self._buffers[self.caps_enc_outputs[i]], _ = self.text_encoder(
                        c_embedding, None, training
                    )
        ...
        dec_output, _ = self.decoder(
            ...
            self._buffers["text_enc_output"],
            [
                self._buffers[self.caps_enc_outputs[0]],
                self._buffers[self.caps_enc_outputs[1]],
                self._buffers[self.caps_enc_outputs[2]],
            ],
            None,
            self._buffers["text_enc_mask"],
            None,
    ...
    def beam_search(
        ...
        **kwargs
        ...
        return bs.apply(visual, cls, caps, seqs_inp, out_size, return_probs, **kwargs)
