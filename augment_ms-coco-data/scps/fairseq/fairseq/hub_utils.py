...
class GeneratorHubInterface(nn.Module):
    ...
    def sample(self, sentences: List[str], beam: int = 1, verbose: bool = False, **kwargs) -> List[str]:
        ...
        return [(self.decode(hypos[0][0]['tokens']), hypos[1]) for hypos in batched_hypos]
    ...
    def generate(
        ...
        **kwargs
        ...
        for batch in self._build_batches(tokenized_sentences, skip_invalid_size_inputs):
            ...
            batch = utils.move_to_cuda(batch) if self.use_cuda else batch
            translations = self.task.inference_step(
                generator, self.models, batch, **inference_step_args
            )
            for id, hypos in zip(batch["id"].tolist(), translations):
                log_probs = hypos[0]['score'] / math.log(2)
                scores = math.exp(log_probs) / (1+math.exp(log_probs))
                results.append((id, hypos, scores))

        # sort output to match input order
        outputs = [(hypos, scores) for _, hypos, scores in sorted(results, key=lambda x: x[0])]
