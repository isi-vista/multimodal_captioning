...
def compute_scores(trans):
    ref, hyp = [], []
    for image in trans:
        ref.append(" ".join(trans[image][0]))
        hyp.append(" ".join(trans[image][1]))

    assert len(ref) == len(
        hyp
    ), "The number of reference and transcription sentences does not match (%d vs. %d)".format(
        len(ref), len(hyp)
    )
    ...
    scores = {"CER": error, "WER": "N/A", "SER": "N/A"}
