...
def get_data(_to_eval_directory, _to_eval_file, _transform):
    ...
    if _to_eval_file is not None:
        assert (
            os.path.splitext(_to_eval_file)[1].lower() in available_extensions
        ), f"{_to_eval_file} 格式不支持"
        ...
        assert os.path.exists(_to_eval_directory) and os.path.isdir(
            _to_eval_directory
        ), f"{_to_eval_directory} 文件夹无效"
...
def plot_detect_result_on_img(_img, _polygons):
    ...
    for m_polygon_index, m_polygon in enumerate(_polygons):
        ...
        to_draw.text(
            m_polygon[0],
            f"{m_polygon_index}",
            font=default_font_for_annotate,
            fill="blue",
        )
...
def mask2polygon(_mask):
    ...
    contours, _ = cv2.findContours(
        (_mask * 255).astype(np.uint8), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE
    )
...
def clockwise_points(_point_coords):
    ...
    center_point = tuple(
        map(
            operator.truediv,
            reduce(lambda x, y: map(operator.add, x, y), _point_coords),
            [len(_point_coords)] * 2,
        )
    )
    return sorted(
        ...
        key=lambda coord: (
            135
            - math.degrees(
                math.atan2(*tuple(map(operator.sub, coord, center_point))[::-1])
            )
        )
        % 360,
    )
...
def assign_points_to_four_edges(_point_coords):
    ...
    for i in range(1, max_distance_index):
        # 计算每个两边之间的夹角
        m_angle_1 = math.atan2(
            points_np[i][1] - points_np[0][1], points_np[i][0] - points_np[0][0]
        )
        m_angle_2 = math.atan2(
            points_np[i][1] - points_np[max_distance_index][1],
...
def extract_rectangle_with_correct_aspect_ratio(_img, _four_corner_points):
    ...
    v0 = rows / 2.0
    p = [
        _four_corner_points[0],
        _four_corner_points[1],
        _four_corner_points[3],
        _four_corner_points[2],
    ]
...
def polygon_to_rectangle_basic(_img, _polygon, _aspect_ratio_correct=True):
    ...
    if not _aspect_ratio_correct:
        M = cv2.getPerspectiveTransform(
            [m_edge[0] for m_edge in fours_edges],
            [[0, 0], [first_edge, 0], [first_edge, second_edge], [0, second_edge]],
        )
        warped_roi = cv2.warpPerspective(img_np, M, (first_edge, second_edge))
    else:
        warped_roi = extract_rectangle_with_correct_aspect_ratio(
            img_np, [m_edge[0] for m_edge in fours_edges]
        )
...
def recognition_result_post_process(
    _recognition_result, _recognizer_model_type, _str_label_converter
):
    """
...
if __name__ == "__main__":
    ...
    def _resize_img_for_detect(_img, _longer_edge_length, _shorter_edge_base_length):
        ...
        if h > w:
            ...
            new_w = (
                (new_h / h * w) // _shorter_edge_base_length * _shorter_edge_base_length
            )
        else:
            new_w = _longer_edge_length
            new_h = (
                (new_w / w * h) // _shorter_edge_base_length * _shorter_edge_base_length
            )
    ...
    with torch.no_grad():
        for m_path, m_pil_img, m_eval_tensor in tqdm(
            ...
            m_polygons = detect_result_post_process(
                m_detect_result, detector_model_type
            )
            m_recognized_results = []
            # 提取所有的文本区域
            for m_region_tensor in extract_tensor_for_recognize(
                m_pil_img, m_polygons, eval_recognize_transformer
            ):
                refined_recognized_result = recognition_result_post_process(
                    recognizer(m_region_tensor), recognizer_model_type, label_converter
                )
                m_recognized_results.append(refined_recognized_result)
            m_final_polygons, m_final_text = related_polygon_assembly(
                m_polygons, m_recognized_results
            )
            if annotate_on_image:
                ...
                with open(
                    f"{m_base_name}_result.txt", mode="w", encoding="utf-8"
                ) as to_write:
