...
def main(argv=None):
    ...
    with open(args.input_text_file, "rb") as fh1, open(args.output_text_file, "wb") as fh2:
        t1, t2, t3, t4, t5, t6 = pickle.load(fh1)

        tr1,tr2,tr3,tr4,tr5,tr6 = [], [], [], [], [], []
        for i, item in enumerate(zip(t1, t2, t3, t4, t5, t6)):
            name = splitext(basename(item[0][0]))[0]
            if name in keys:
                tr1.append( (name,) + item[0][1:] )
                tr2.append( item[1] )
                tr3.append( (name,) + item[2][1:] )
                tr4.append( (name,) + item[3][1:] )
                tr5.append( item[4] )
                tr6.append( (name,) + item[5][1:] )

        pickle.dump([tr1, tr2, tr3, tr4, tr5, tr6], fh2, protocol=pickle.HIGHEST_PROTOCOL)
